...
// import { Camera, CheckCircle, AlertCircle, Upload, X, MapPin, User, Clock } from 'lucide-react';

// const Form = ({ user, selectedLocation, onBack, onLogout }) => {
...
//   const baseUrl = 'http://localhost:3002'; // Your API base URL

//   // Fetch checklist data from API
//   const fetchChecklistData = async () => {
//     try {
//       const response = await fetch(`${baseUrl}/api/doctor/doc-checklist-tasks`);
...
//     try {
//       const response = await fetch(`${baseUrl}/api/doctor/support-subcategories`);
//       const result = await response.json();
...
//     // Here you can add API call to record check-in
//     console.log('Checked in at:', new Date().toLocaleString());
//   };

//   // Handle check-out
//   const handleCheckOut = () => {
//     // Here you can add API call to record check-out
//     console.log('Checked out at:', new Date().toLocaleString());
//     alert('Checked out successfully!');
//     onBack(); // Go back to location selection
//   };
...
//                 <p className="text-blue-100 mt-2">Complete the checklist and document support issues</p>
//               </div>
//               <div className="text-right">
//                 <button
//                   onClick={onBack}
//                   className="text-blue-100 hover:text-white transition-colors mb-2 block"
//                 >
//                   ← Back to Locations
//                 </button>
//                 <button
...
//                     {selectedLocation?.farmData && ` - ${selectedLocation.farmData.farm.FarmName}`}
//                   </p>
//                 </div>
//               </div>
//               <div className="flex items-center">
...
//               <h2 className="text-2xl font-semibold text-gray-800 mb-4">Ready to Start Your Visit?</h2>
//               <p className="text-gray-600 mb-6">Check in to begin documenting your visit</p>
//               <button
//                 onClick={handleCheckIn}
//                 className="bg-gradient-to-r from-green-500 to-emerald-600 text-white py-4 px-8 rounded-xl font-semibold text-lg hover:from-green-600 hover:to-emerald-700 transform hover:scale-105 transition-all duration-200 shadow-lg"
//               >
//                 📍 Check In Now
//               </button>
//             </div>
...
//                   </div>
//                   <h2 className="text-2xl font-semibold text-gray-800">Checklist Tasks</h2>
//                   <span className="bg-blue-100 text-blue-800 text-sm px-2 py-1 rounded-full">
//                     {checklistData.length} tasks
//                   </span>
//                 </div>
...
//                           <span className="font-medium text-gray-800">{item.ChecklistTask}</span>
//                           <div
...
//                   </div>
//                   <h2 className="text-2xl font-semibold text-gray-800">Support Issues</h2>
...
//                         {Object.entries(groupedSupportData).map(([categoryName, subcategories]) => (
//                           <button
//                             key={categoryName}
//                             type="button"
...
//                     {/* Subcategories for Active Category - Vertical Layout */}
...
//                                       <button
//                                         type="button"
//                                         onClick={() => removeImage(subcategory.id)}
...
//                                         type="file"
//                                         accept="image/*"
//                                         onChange={(e) => handleImageUpload(subcategory.id, e.target.files[0])}
//                                         className="hidden"
...
//                                       {images[subcategory.id] ? 'Change Image' : 'Select Image'}
//                                     </label>
//                                   </div>
//                                 </div>
...
//                                   value={supportFormData[subcategory.id]?.observation || ''}
//                                   onChange={(e) => handleSupportFormChange(subcategory.id, 'observation', e.target.value)}
//                                 />
//                               </div>
//                             </div>
//                           </div>
...
//                   className={`px-8 py-4 rounded-xl font-semibold text-lg shadow-lg transform transition-all duration-200 ${
...
// import React, { useState, useEffect } from 'react';
// import { Camera, CheckCircle, AlertCircle, Upload, X, MapPin, User, Clock, ArrowLeft } from 'lucide-react';

// const Form = ({ user, selectedLocation, activeTravel, onBack, onLogout }) => {
//   const [checklistData, setChecklistData] = useState([]);
//   const [supportData, setSupportData] = useState([]);
//   const [loading, setLoading] = useState(true);
//   const [error, setError] = useState(null);
...
//       // Here you can add API call to submit form data
//       // const response = await fetch(`${baseUrl}/api/doctor/submit-form`, {
//       //   method: 'POST',
...
//       alert('Visit report submitted successfully!');
//       onBack(); // Go back to check-in/travel page
//     } catch (err) {
...
//                   <p className="font-medium text-gray-900">
//                     {activeTravel ? formatDate(activeTravel.checkinTime) : 'N/A'}
//                   </p>
//                 </div>
//               </div>
//               <div className="flex items-center">
...
//                   <p className="font-medium text-gray-900">#{activeTravel?.id || 'N/A'}</p>
//                 </div>
//               </div>
//             </div>
//           </div>
...
//                 type="button"
//                 onClick={onBack}
//                 className="flex items-center bg-gray-500 text-white px-6 py-3 rounded-lg font-medium hover:bg-gray-600 transition-colors"
//               >
...
//       const response = await fetch(`${baseUrl}/api/doctor/checkout`, {
//         method: 'POST',
//         headers: {
...
//         onBack(); // Go back to travel history
//       } else {
...
//     try {
//       // Prepare categories data in the required format
//       const categoriesData = {};
      
//       // Group support form data by category
//       Object.entries(supportFormData).forEach(([subcategoryId, data]) => {
//         const subcategory = supportData.find(item => item.id === parseInt(subcategoryId));
...
//           categoriesData[categoryId].subcategories.push({
//             id: parseInt(subcategoryId),
//             observation: data.observation || '',
//             status: data.observation ? 'Completed' : 'Pending',
//             image: images[subcategoryId] ? 'uploaded' : null // You can upload to server and get URL
//           });
//         }
//       });
      
//       const formData = {
//         FarmId: selectedLocation?.farmData?.FarmId || null,
//         LocationId: selectedLocation?.locationId || null,
//         remark: `Visit completed by ${user?.username}. Checklist items: ${selectedChecklistItems.length} completed.`,
//         FormDetail: {
//           categories: Object.values(categoriesData),
//           checklistItems: selectedChecklistItems // Add checklist items to the submission
//         }
//       };
...
//             <div className="flex justify-between items-start">
//               <div>
//                 <h1 className="text-xl font-bold text-white">Farm Visit Report</h1>
//                 <p className="text-blue-100 text-sm mt-1">Complete checklist and document issues</p>
//               </div>
//               <div className="text-right">
//                 <button
//                   onClick={onBack}
...
//                   <p className="font-medium text-gray-900 text-xs">#{activeTravel?.id || 'N/A'}</p>
//                 </div>
//               </div>
//             </div>
//           </div>
...
//                   <CheckCircle className="h-6 w-6 text-blue-600" />
//                 </div>
//                 <h2 className="text-2xl font-semibold text-gray-800">Top Pointers</h2>
...
//                     <p className="text-green-800 font-medium text-sm">Form submitted successfully!</p>
//                     <p className="text-green-600 text-xs">You can now checkout from this location</p>
//                   </div>
