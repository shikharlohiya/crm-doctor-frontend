...
const CheckInTravel = () => {
  ...
  // Fetch travel data (only when online)
  const fetchTravelData = async () => {
    if (!networkStatus.isOnline) {
      console.log("Skipping travel data fetch - offline");
      return;
    }
    ...
    try {
      setTravelLoading(true);
      const response = await axiosInstance.get(
        `/doctor/daily-travel-info?EmployeeId=${user.EmployeeId}`
      );
      const result = response.data;
      if (result.success && result.data && Array.isArray(result.data.travels)) {
        setTravelData(result.data.travels);
  ...
  return (
    <div className="">
      <div className="max-w-md mx-auto ">
          ...
          <div className="flex justify-between items-center text-xs">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-1">
                ...
                <span className={networkDisplay.color}>
                  {networkDisplay.text}
                </span>
              </div>
              ...
              <div className="flex items-center space-x-1">
                {pendingSyncCount > 0 ? (
                  ...
                  <Sync className="h-3 w-3 text-green-500" />
                )}
            ...
            {/* Manual Sync Button */}
            {syncDisplay.showSync && networkStatus.isOnline && (
              <button
                onClick={handleSync}
        ...
        {/* Offline Notice */}
        {!networkStatus.isOnline && (
          <div className="mx-4 mt-4 bg-orange-50 border border-orange-200 rounded-lg p-3">
            <div className="flex items-center">
              <CloudOff className="h-4 w-4 text-orange-500 mr-2" />
              <div>
        ...
        <div className="flex-1 p-4">
          {!isCheckedIn ? (
            ...
            <div className="text-center space-y-4">
              ...
              <p className="text-gray-600 text-sm">
                Check in to begin documenting your visit.{" "}
                {!networkStatus.isOnline
              ...
              >
                {checkInLoading ? (
                  ...
                  <>📍 Check In Now{!networkStatus.isOnline && " (Offline)"}</>
                )}
              </button>
              ...
              {!networkStatus.isOnline && (
                <div className="text-xs text-gray-500">
                  ✓ Location can be captured offline
                  <br />✓ Data will sync when connection returns
                </div>
              )}
            </div>
          ) : (
              ...
              {!networkStatus.isOnline ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-500 mb-4">
                    Travel history unavailable offline
                  </p>
                  <button
                    onClick={handleStartForm}
                ...
                <div className="text-center py-8 text-gray-500">
                  <Calendar className="h-8 w-8 mx-auto mb-2 text-gray-400" />
                  <p className="text-sm mb-4">No travel records found</p>
                  <button
                    onClick={handleStartForm}
                    className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700 transition-colors"
                  >
